...
def build_prompt(request: ModelRequest) -> str:
  return _build_prompt_simple(request=request)


def _build_prompt_simple(request: ModelRequest) -> str:
    return dedent(
        f"""
        Find the object described by the query in the image and estimate its 2D and 3D bounding box.

        Query: {request.query}
        Image: {request.width}x{request.height} pixels
        ...
        3. box_3d: camera frame is x-right, y-down, z-forward. z_center_mm must be positive. All sizes in mm and strictly positive.
           ...
           - fx = {request.intrinsics[0]:.1f} (from the intrinsics above).
           - Example: object spans 200 norm units tall on a {request.height}px image → pixel_span = 0.2 × {request.height} = {int(0.2 * request.height)} px; if real height ≈ 120 mm → z ≈ {request.intrinsics[0]:.0f} × 120 / {int(0.2 * request.height)} ≈ {int(request.intrinsics[0] * 120 / max(1, int(0.2 * request.height)))} mm.
